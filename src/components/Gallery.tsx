    ...
    <section className="py-12 container mx-auto px-4 border-b-2 border-[#C1CFD6] relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {galleryImages.map((image) => (
          ...
          >
            <div className="aspect-[4/3] overflow-hidden bg-[#E8E8E8] flex items-center justify-center p-4">
              <img
                src={image.url}
                alt={image.title}
            ...
            <div className="p-3 bg-white border-t-2 border-[#C1CFD6]">
              <p className="text-[#2E2E4E] text-sm mb-3" style={{ fontFamily: 'Calibri, sans-serif' }}>
                {image.title}
              </p>
              <Button
      ...
      <Dialog open={showCartModal} onOpenChange={setShowCartModal}>
        <DialogContent className="border-2 border-[#C1CFD6] bg-white max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-[#2E2E4E]" style={{ fontFamily: 'Arial, Helvetica, sans-serif', fontWeight: 'bold' }}>Shopping Cart</DialogTitle>
          </DialogHeader>

          {cartItems.length === 0 ? (
            <div className="py-8 text-center border-2 border-dashed border-[#C1CFD6] rounded-lg">
              <p className="text-[#2E2E4E]" style={{ fontFamily: 'Calibri, sans-serif' }}>Your cart is empty.</p>
            </div>
          ) : (
            <>
